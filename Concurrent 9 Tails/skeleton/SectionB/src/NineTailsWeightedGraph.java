...
public class NineTailsWeightedGraph {
	...
	private void constructGraph() {
		...
		// graph of form: child -> edges with child as child.
		configurations = new ListArrayBased<>();
		IntStream.range(1,NUM_CONFIGURATIONS + 1).forEach(index -> configurations.add(index, new PriorityQueue<>()));
		for (int config = 1; config <= NUM_CONFIGURATIONS; config++){
			for (Iterator<Object> it = generateParents(config).iterator(); it.hasNext(); ) {
